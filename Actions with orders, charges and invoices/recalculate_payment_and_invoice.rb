...
@order = Order.find(order_id)
  ...
  def recalculate_invoice
    ...
    if invoice
      if !invoice.closed? && invoice.payment.status = 'waiting_for_payment' && !invoice.approval_date?
        ...
        puts "Invoice status: #{invoice.status}, Payment status: #{invoice.payments.last.status}"
      end
    else
