...
counter = 0
notes = Note.where(noteable_type: "Order").where(manager_id: [2325, 4566, 4768, 4674, 1862, 4384, 4336, 4713, 597]).select { |note| note.content.include? 'completed manually' }
...
subs_ids_for_fix = Subscription.where(id: @subs_id_where_not_actual_price).where("updated_at > ?", Date.new(2021,1,4)).where.not(status: :deleted).ids
