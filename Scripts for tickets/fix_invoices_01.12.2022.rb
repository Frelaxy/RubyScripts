module BillingProcesses
  ...
  def fix_october_invoices()
    ...
    charges = Charge::Base.where(subscription_id: subs_ids).where("operate_from = ? and operate_from < ?", Date.new(2022,10,1), Date.new(2022,11,1)).where("close_date = ?", Date.new(2022,11,1)).where.not(status: :deleted)
    charges.map do |charge|
      ...
      if payment.nil?
        ...
        p "Payment #{payment.id} in status #{payment.status}"
        @log_payment << payment
      end
      Note.create!(
